...
- **Interactive** is a captain-driven guided operation in a dedicated hands-off pane, the default for a prod-write walkthrough or multi-step repair the captain must perform by hand (a repair script, a resend or refund flow, a console delete, wizard-style provisioning). The captain performs every action while an in-pane agent hands the captain the exact next step, verifies each result, and advances. The agent makes no product edits and drives no pipeline, and being human-driven grants no expanded merge, destructive, or approval authority. Scaffold it with `bin/fm-brief.sh <id> <repo> --interactive` and spawn it with `bin/fm-spawn.sh <id> projects/<name> --interactive` (records `kind=interactive`, implies the hands-off `--unsupervised` pane), then have the captain switch into the pane. Its deliverable is a session log at `data/<id>/report.md` and its worktree is scratch, so it reports status and tears down exactly like a scout. A quick one-off read or single answer stays in the main session, an autonomous code change is a ship, and a pure investigation is a scout.
...
3. For `check:`, act on the named poll result, including merges, X-mode events, a `host-resources` reading, the hourly `session-review` and `session-cleanup` passes, and a `context-stow-nudge` (firstmate's own context crossed the stow threshold: `/stow`, then `/compact`, then re-arm supervision, before auto-compaction discards un-stowed knowledge; owned by `docs/configuration.md` "Firstmate own-context stow threshold").
   A `model-drift <id>` wake means the jcode session store disagrees with the profile the meta records for a live lane; run `bin/fm-jcode-repin.sh <id>` to re-pin the recorded model and effort through jcode's race-free debug socket and re-verify against the store, or escalate to the captain with the wanted and actual values from the wake line (mechanism owned by `bin/fm-jcode-profile-lib.sh`, `bin/fm-spawn.sh`, and `bin/fm-watch.sh`).
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `NM_SANDBOX:`, `CREW_DISPATCH: invalid`, `SKILLS_MANIFEST:`, `FLEET_SYNC:`, `PRESENT_DAEMON:`, `LIVENESS_WATCHDOG:`, `LIVENESS_ESCALATION:`, `PR_CHECK_MIGRATION:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `AFK_DAEMON:`, `AFK_READER:`, `NUDGE_SECONDMATES:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
