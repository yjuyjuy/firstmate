...
config/token-prices.json  shared token-price snapshot, TRACKED (the one non-local config file); written ONLY by bin/fm-token-prices.sh --refresh as a straight copy of providers.anthropic out of jcode's cached models.dev feed, with a traceable price_source/cached_at/written_at header; never hand-edited (bin/fm-token-prices.sh header owns the format; design PR-T1)
