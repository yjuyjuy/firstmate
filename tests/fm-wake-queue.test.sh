...
test_signal_catchup_without_running_watcher() {
  ...
  # 200-tick (20s) ceiling, not a timing assumption: races the watcher's real
  # first-cycle startup cost under host load (same reasoning as the stale-pane
  # exit wait below and in fm-watch-triage.test.sh).
...
test_stale_enqueue_before_suppressor() {
  ...
  # 200-tick (20s) ceiling, not a timing assumption: the watcher's first cycle
  # (sleep POLL + signal-coalesce linger + the scan) takes ~4.4s under heavy
  # load, just past the earlier 40-tick (4s) budget, so this exit-wait raced
  # the watcher and flaked with no watcher defect (same reasoning documented in
  # fm-watch-triage.test.sh for its identical 200-tick ceilings).
  wait_for_exit "$!" 200 || fail "watcher did not exit for stale pane"
...
test_not_working_stale_enqueue_before_suppressor() {
  ...
  # 200-tick (20s) ceiling: see the signal-catchup test above for why 40 ticks
  # (4s) raced the watcher's real first-cycle startup cost under host load.
  wait_for_exit "$!" 200 || fail "watcher did not surface a not-provably-working stale"
...
SH
  ...
  wait_for_exit "$!" 200 || fail "watcher did not exit for check output"
