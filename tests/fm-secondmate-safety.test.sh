...
test_fm_home_parameterization() {
  ...
  [ "$out" = "local-only on off off" ] || fail "fm-project-mode did not read projects.md from FM_HOME"
  out=$(FM_HOME="$home_two" "$ROOT/bin/fm-project-mode.sh" app 2>/dev/null)
  [ "$out" = "no-mistakes off off off" ] || fail "fm-project-mode did not isolate missing registry by home"
  ...
  grep -F "fm-status-append.sh '$home_one/state/task-a.status'" "$brief" >/dev/null || fail "brief did not shell-quote FM_HOME state path"
  ...
  grep -F "fm-status-append.sh '$home_one/state/task-b.status'" "$brief" >/dev/null || fail "scout brief did not shell-quote FM_HOME state path"
  ...
  grep -F "fm-status-append.sh '$home_one/state/task-c.status'" "$brief" >/dev/null || fail "secondmate brief did not shell-quote FM_HOME state path"
