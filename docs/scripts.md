...
| `fm-secondmate-context.sh` | Read-only report of a secondmate's context-window usage against the handoff threshold (claude and jcode; else unknown) |
...
| `fm-project-mode.sh`     | Resolve a project's delivery mode and `+yolo`/`+autoland`/`+no-ci` flags from `data/projects.md` |
...
| `fm-afk-driver.sh`       | Advance the away-mode queue autonomously and safely: clean up finished lanes (holding any whose no-mistakes delivery run is still active and pre-PR), nudge one that never pushed, start prepared queued work, report every action |
...
| `fm-status-append.sh`    | The single capped write point for crew status appends: a line at/under the cap (default 300, `FM_STATUS_APPEND_CAP` overrides) is appended unchanged; a longer line spills its full body to `data/<id>/status-overflow/<ts>.txt` first, then appends a truncated line suffixed `... [full: <path>]`, keeping the state-verb prefix at the front so `fm-classify-lib.sh` triage is unaffected. It also caps the whole file: after each append, if the live `state/<id>.status` exceeds `FM_STATUS_FILE_MAX_LINES` (default 200, 0 disables) it archives the oldest lines to `state/<id>.status.1` then atomically trims the live file to its most-recent MAX lines, preserving the verb-keyed tail readers depend on while nothing is discarded |
...
| `fm-merge-queue-lib.sh`  | Own the `data/merge-queue.tsv` format, locked record/remove writes, the content-in-base merged check plus its GitHub forge-confirmed fallback, and the queue-vs-live-meta drift reconcile |
...
| `fm-token-sessions-lib.sh` | Side-effect-free owner of the append-only `data/token-sessions.tsv` ledger mechanics, `fm_resolve_crew_session_id` (newest-created_at jcode session matching leased worktree and spawn-or-later `created_at`, fail-closed to empty), `fm_harness_session_store_readable` (single named owner of which harnesses have a readable session store; jcode only today), `fm_token_sessions_capture` (post-launch attribution helper called on EVERY spawn regardless of harness: skips an unreadable-store harness, else retries the resolve while the store settles asynchronously bounded by `FM_TOKEN_CAPTURE_RETRIES`/`FM_TOKEN_CAPTURE_SLEEP`, records, verifies the write by reading the exact id+session_id back, and on give-up logs one diagnostic instead of silently dropping the row; best-effort, never blocks a spawn), and `fm_token_sessions_rows_for` (all ledger rows for a task id, mirrors `fm_completions_lookup`; design PR-T4); appended best-effort from `fm-spawn.sh` after every launch and from `fm-session-start.sh` under the sentinel id `__firstmate__` |
| `fm-resume-lib.sh`       | Side-effect-free owner of the harness resume-command mapping (`fm_resume_command`: jcode/grok/codex to their verified by-id `--resume` forms, everything else fail-closed, shell-unsafe tokens refused) and `fm_resume_token_for_harness` (a jcode session id is its own resume token); mirrors only the by-id resume forms the harness-adapters skill verifies |
| `fm-resume-cmd.sh`       | Print the exact resume command for a stuck/dead crewmate task from its recorded `harness=` + `resume=` meta, so recovery resumes the session in place instead of restarting; fail-closed with a clear diagnostic and non-zero exit when there is no meta, no captured resume token, or no verified by-id resume command (stuck-crewmate-recovery) |
...
| `fm-token-tier-lib.sh` | Side-effect-free single owner of the model->spend-tier mapping: sourced (never executed), it matches a lowercased model id against the shell-glob patterns in the tracked snapshot `config/model-tiers.json` (path overridable via `$FM_MODEL_TIERS`) in array order, first tier wins, an unmatched model lands in the config's `default_tier` ("other"); `fm-token-report.sh --by-tier` is its only caller so a new model is tiered by editing the JSON, never code |
| `fm-token-report.sh`  | Read-only per-session, per-period, and time-bucketed (`--by hour\|day\|week\|month`, `--precise` for per-message bucketing) token/cost report over `$JCODE_SESSIONS_DIR`; groups optionally `--by-model`/`--by-provider`/`--by-tier` (the latter maps each model to its spend tier via `fm-token-tier-lib.sh` reading `config/model-tiers.json`, adding a distinct-ticket count per tier), supports `--json`; aggregates raw token sums per group and costs every group through `fm-token-lib.sh` (`--session` routes through `fm_token_sum_session` for exact PR-T1 parity), never reimplementing dollar math; unknown-model tokens land in a separate bucket with dollars withheld, never a fabricated $0 (design PR-T2); adds a per-ticket dimension: bare `<task-id>` sums every `token-sessions.tsv` ledger row for that id (EXACT), `--period ... --by-ticket` groups every store session by ticket, resolving each session through the captain-approved attribution sources in precedence order (the home's own ledger, then the no-mistakes run behind an nm worktree path via a READ-ONLY `state.sqlite` read, then a registered secondmate home's own ledger, then a coarse `[spawn_ts, next-spawn_ts)` window over ledgered worktrees) and labelling each ticket line `exact` for the keyed joins or `ESTIMATE` for the window join per captain decision D4, with anything unmatched left in the `unattributed` bucket, and `<task-id> --retro` estimates pre-capture tickets from a `completions.tsv`-close-date-bounded window, always labeled ESTIMATE and refusing tickets with exact ledger data (D4, design PR-T4) |
| `fm-ticket-cost-rollup.sh` | Read-only completions-anchored per-ticket cost join for the work-report skill: walks `data/completions.tsv`, keeps tickets whose close date is in the `--since`/`--until` window (since inclusive, until exclusive; `--repo` scopes to one project), joins each to its `data/token-sessions.tsv` sessions, and prints ONE line per landed ticket (costliest first) with its `cost_if_api` + covered/api split, `--json` for machine output; every dollar comes from `fm-token-lib.sh` so the number is byte-identical to `fm-token-report.sh <task-id>` (single coster owner, no drift); a pre-capture landed ticket with no ledger rows shows `cost n/a`, never $0, and an unpriced model withholds dollars |
