...
| Submit confirmation (idle baseline) | `herdr agent get <pane>` -> `.result.agent.agent_status` after Enter; if the whole budget passes never submit-active, `fm_backend_herdr_composer_state` decides | `fm_backend_herdr_send_text_submit` records the pre-Enter status and, when it is idle/done, confirms delivery by polling for `working`/`blocked` across the Enter attempt's confirmation budget. Only when the budget passes with the agent never submit-active (the LOCAL-slash class - a consumed /effort leaves agent_status and state_change_seq frozen, verified 2026-09-03) does the path consult the composer: emptied = confirmed, still-held = retry, unreadable = pending without another Enter. Composer-state reads remain the affirmative-empty pre-injection guard and the conservative fallback for preexisting submit-active or unreadable baselines; see "Native agent-state submit confirmation". |
| Bounded capture | `herdr pane read <pane> --source recent --lines N` | See "Verified bug" below - N is never passed through directly. |
| ANSI capture | `herdr pane read <pane> --source recent --lines N --format ansi` | Herdr 0.7.3 preserves composer de-emphasis styling, letting the shared `fm_composer_strip_ghost` extractor treat dim/faint and dark-TRUECOLOR ghost/placeholder text as empty while retaining real typed input. The same small-`--lines` workaround applies. Since 2026-09-03 the adapter trims trailing blank rows before tailing, because herdr 0.8.0 pads the ansi read to the full viewport height and a freshly spawned jcode pane leaves blank rows BELOW its composer (see "Local-slash confirmation and the blank-tail composer read"). |
...
Correction (2026-09-03, task r5-herdr-jcode-send-verdict, live herdr 0.8.0 + jcode v0.81.69-dev): since jcode v0.68.7, modern jcode builds report their agent state to herdr NATIVELY - `herdr agent get <pane>` returns `agent_status: working|idle|done` plus a monotonically increasing `state_change_seq` for every live jcode pane (verified live against this fleet's own jcode panes, including a freshly spawned one). The 2026-08-01 claim above remains true only for pre-0.68.7 jcode builds, and the content-corroboration path below stays as defense-in-depth for those builds and for the genuinely agent-less husk shape, but the submit path and the liveness probe now see a REAL registered status for current jcode.
...
One deliberate exception exists since task r5-herdr-jcode-send-verdict (2026-09-03): a LOCAL slash command never starts a model turn, so `agent_status` can never flip to `working` for it even when it DID land and execute.
Verified live against jcode v0.81.69-dev + herdr 0.8.0: a consumed `/effort medium` left BOTH `agent_status` (idle) and `state_change_seq` (1742 before and after) frozen while the command executed and the composer cleared, so neither native field is a usable submit signal for the local-slash class.
The one truthful signal is the composer EMPTYING, so when an idle baseline's whole confirmation budget passes with the agent never submit-active, `fm_backend_herdr_send_text_submit` consults `fm_backend_herdr_composer_state`: an emptied composer confirms delivery (this also closes the instant-round-trip residual gap below), a still-held composer keeps the retry loop honest, and an unreadable composer fails closed immediately with the never-retry-past-unreadable invariant.
A submit-active `working`/`blocked` read still short-circuits BEFORE any composer read, so the 2026-07-07 codex-idle-tip hazard cannot regress (an EMPTY composer read is only ever produced by the shared ghost-aware classifier stripping text that is NOT the typed steer).
...
- **Instant round-trip.** A turn that starts and returns to idle entirely between two polls would, in the limit, never show as submit-active at all.
  ...
  On the residual chance this happens, the r5 composer fallback now reads the thus-emptied composer as `empty` and confirms delivery; without that fallback the verdict used to be `pending`, and the caller's own invariant (retry Enter only, never retype) meant the worst case was a redundant Enter landing on an already-empty composer - a no-op, not a duplicate delivery of the message text.
...
### Local-slash confirmation and the blank-tail composer read (2026-09-03, jcode v0.81.69-dev + herdr 0.8.0)

Task r5-herdr-jcode-send-verdict verified the JCODE side of submit confirmation live against real jcode in a throwaway herdr workspace (`herdr agent start` + `herdr workspace close` for cleanup), on this fleet's own herdr 0.8.0 server:

- **A consumed local slash leaves agent_status AND state_change_seq frozen.** With the pane genuinely idle (baseline `agent_status: idle`, `state_change_seq: 1742`), `/effort medium` was typed and Enter sent. The command executed ("✓ Reasoning effort → Medium" in the transcript) and the composer cleared, yet every poll across ~2.5s kept reporting `agent_status: idle` and `state_change_seq: 1742` - neither native field moves for a local slash, so the old "wait for working" confirmation could never confirm this class and reported the false "Enter swallowed; text left in composer" (`pending`) while the slash had actually landed (this was the reported fleet symptom: `/effort`, `/model`, `/no-mistakes`, `/account` steers all printing the false error).
  For contrast, a REAL text turn ("Reply with exactly: OK") flipped `agent_status` to `working` and bumped `state_change_seq` 1742 -> 1746 within ~0.9s, so the native working-transition signal remains correct for model turns.
- **An emptied composer is the truthful local-slash signal, and the fix confirms delivery from it.** After the fix, the same live `/effort medium` send reports `empty` (confirmed) with exactly ONE Enter; the pane tail shows the composer back at the idle `1>` row with the command's output in the transcript.
- **The herdr 0.8.0 ANSI pane read pads to the full viewport height, and a freshly spawned jcode pane draws its UI above the buffer bottom.** Live: the composer row of a fresh jcode pane sat ~30 rows above the bottom of the 80-row read buffer, with blank rows below it. `fm_backend_herdr_capture`/`capture_ansi` previously fetched generously but then tailed to the caller's bound, so `fm_backend_herdr_composer_state`'s 20-line window started inside the blanks and read `unknown` on a LIVE jcode composer (both idle-empty and with real typed text). The capture functions now trim trailing blank rows BEFORE the tail window, so the bottom-most content row (the composer) is the window's last row; a viewport-filling pane (no trailing blanks) is byte-for-byte unchanged. This also fixes the fleet-wide `unknown` composer verdict that made the account-switch broadcast and the away-daemon's pre-injection guard skip fresh jcode panes.
- **The unreadable direction still fails closed.** With the composer made unreadable, a local-slash send reports `pending` after exactly one Enter (no retry past an unreadable pane).
