...
The drain script calls that guard after emptying the queue, which avoids repeating the queued-wakes warning for records it just consumed while still warning on stale watcher liveness; the guard likewise drops that advisory when every pending record is a benign already-surfaced signal (its status signature still matches the watcher's `.seen-*` marker and its last line is not captain-relevant), since draining would surface nothing new, while a read-only session still reports the queue to the lock holder.
