...
`data/projects.md` records each project's delivery mode and optional `+yolo` autonomy, `+autoland` self-land, and `+no-ci` fork-no-CI flags.
...
The `+no-ci` flag (orthogonal to mode and the other flags) marks a fork or no-merge-authority repo whose forge reports no CI on our branches, so a `no-mistakes` lane can never reach "CI green": the generated brief tells the crew to park on a clean, mergeable PR with an explicit `paused: ... awaiting captain merge (no CI on fork)` terminal line instead of chasing an unreachable green, which lets the watcher absorb the idle pane on its long cadence rather than wedge-escalating it as a stopped crew.
