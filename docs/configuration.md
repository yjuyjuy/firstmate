...
`config/` holds local gitignored operating choices - with two tracked exceptions, the shared token-price snapshot `config/token-prices.json` owned by `bin/fm-token-prices.sh` (`--refresh`, see that script's header and `AGENTS.md` section 2) and the shared model-to-spend-tier map `config/model-tiers.json` owned by `bin/fm-token-tier-lib.sh` - and `projects/` holds the local project clones that Firstmate reads but changes only through the guarded exceptions in `AGENTS.md`.
...
config/token-prices.json  shared token-price snapshot, TRACKED (one of two tracked config files); the owned per-provider USD-per-Mtok price table that bin/fm-token-lib.sh costs token usage against; written ONLY by bin/fm-token-prices.sh --refresh as a straight copy of every provider table out of jcode's cached models.dev feed, with a header carrying price_source, the source's cached_at, and the snapshot's written_at so every price is traceable from the file alone; never hand-edited (one-owner-per-contract; bin/fm-token-prices.sh header owns the format, design PR-T1)
config/model-tiers.json  shared model-to-spend-tier map, TRACKED (the other tracked config file); the owned data the tier matcher bin/fm-token-tier-lib.sh reads so bin/fm-token-report.sh --by-tier groups spend by tier (tooling vs product vs other); each tier lists shell-glob patterns matched case-insensitively against the recorded model, first tier wins, an unmatched model lands in default_tier; edit the patterns (not code) to tier a new model (one-owner-per-contract; bin/fm-token-tier-lib.sh header owns the matching contract)
