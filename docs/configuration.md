...
`config/` holds local gitignored operating choices - with three tracked exceptions, the shared token-price snapshot `config/token-prices.json` owned by `bin/fm-token-prices.sh` (`--refresh`, see that script's header and `AGENTS.md` section 2), the shared model-to-spend-tier map `config/model-tiers.json` owned by `bin/fm-token-tier-lib.sh`, and the shared fleet skills manifest `config/skills-manifest` owned by `bin/fm-skills-manifest.sh` - and `projects/` holds the local project clones that Firstmate reads but changes only through the guarded exceptions in `AGENTS.md`.
...
config/token-prices.json  shared token-price snapshot, TRACKED (one of three tracked config files); the owned per-provider USD-per-Mtok price table that bin/fm-token-lib.sh costs token usage against; written ONLY by bin/fm-token-prices.sh --refresh as a straight copy of every provider table out of jcode's cached models.dev feed, with a header carrying price_source, the source's cached_at, and the snapshot's written_at so every price is traceable from the file alone; never hand-edited (one-owner-per-contract; bin/fm-token-prices.sh header owns the format, design PR-T1)
config/model-tiers.json  shared model-to-spend-tier map, TRACKED; the owned data the tier matcher bin/fm-token-tier-lib.sh reads so bin/fm-token-report.sh --by-tier groups spend by tier (tooling vs product vs other); each tier lists shell-glob patterns matched case-insensitively against the recorded model, first tier wins, an unmatched model lands in default_tier; edit the patterns (not code) to tier a new model (one-owner-per-contract; bin/fm-token-tier-lib.sh header owns the matching contract)
config/skills-manifest  shared fleet skills manifest, TRACKED; the owned list of first-party tool skills every firstmate and secondmate home is expected to have, one `<owner>/<repo>@<skill>` source per line, comments and blank lines ignored; read by bin/fm-skills-manifest.sh, which detects the gap at session start and installs it during secondmate seeding (docs/configuration.md "Fleet skills manifest"); ADDITIVE-ONLY, so adding a fleet skill means appending one line and nothing is ever pruned
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  decision-desk-ledger.md  human-readable value ledger for the decision-desk secondmate, one row per routed request with its status and optional overturned annotation; firstmate-private, owned by fm-decision-desk-ledger.sh (route/resolve/overturn/tally/list), created lazily on first route
...
state/               volatile runtime signals; gitignored
  <id>.status        appended by crewmates: "<state>: <note>" wake-event lines, not current-state truth; capped to its most-recent FM_STATUS_FILE_MAX_LINES lines by bin/fm-status-append.sh, which archives the older lines to <id>.status.1
  ...
  <id>.crash-tail    the last ~20 herdr pane lines captured by fm-watch's fm_pane_crash_capture (bin/fm-pane-crash-lib.sh) the moment a tracked task's herdr pane is confirmed dead, alongside one `check: pane-crashed <id>` wake so recovery starts with evidence; herdr-only, meta-gated, idempotent (never re-captured for the same death); removed by teardown
...
## Fleet skills manifest (config/skills-manifest)

The tracked `config/skills-manifest` is the fleet's single list of first-party tool skills every firstmate and secondmate home is expected to have, so a new home does not depend on someone remembering an install command.
Each non-comment, non-blank line is exactly one `<owner>/<repo>@<skill>` source; `#` comments and blank lines are ignored, and any other line is a manifest defect that `bin/fm-skills-manifest.sh` refuses loudly rather than skipping.
Adding a fleet skill means appending one line, and that is the whole procedure.

`bin/fm-skills-manifest.sh` is the single owner of the manifest and its application; its `--help` owns exact subcommand and flag mechanics.
`check` (the default) is detect-only: it reports missing skills on one `SKILLS_MANIFEST:` line and never installs, writes, or fetches anything.
`install` installs the missing skills through `npx skills add ... --agent universal -g`, which lands them in `~/.agents/skills`; it accepts optional skill names to install a subset and refuses a name the manifest does not carry.
`list` prints the resolved sources, one per line, for scripts and for eyeballing the manifest.

Application is additive-only by design, because the install root is shared, live, captain-visible state that also holds skills the manifest does not name.
A skill that is already present is left exactly as it is: never re-downloaded, never overwritten, never pruned, and never reconciled to match the manifest.
So a second `install` is a genuine no-op that touches no file, a captain's own local skill is safe, and there is no `--force` and no sync-to-match mode.
Refreshing an already-installed skill to a newer upstream version is `npx skills update`'s job, not this script's.

The manifest reaches homes through two wirings, deliberately asymmetric.
Session start runs the detect-only `check` from `bin/fm-bootstrap.sh` in both locked and read-only modes, so a gap is always visible but session start never mutates the skills tree behind the captain's back; act on a printed `SKILLS_MANIFEST:` line through the `bootstrap-diagnostics` skill.
Secondmate seeding runs `install` from `bin/fm-home-seed.sh`, so a freshly provisioned secondmate comes up with the fleet's tools already in place and needs no manual step; a failed install there warns but does not roll back an otherwise good home, and the next session start reports the remaining gap.

`FM_SKILLS_MANIFEST` overrides the manifest path and `FM_SKILLS_HOME` overrides the install root; both exist for tests and throwaway roots, and neither is an operating knob.
...
[`bin/fm-afk-reader-check.sh`](../bin/fm-afk-reader-check.sh) gives the next firstmate turn a machine-readable instruction instead: session start runs it as `afk_reader_revive_sweep` ([`bin/fm-bootstrap.sh`](../bin/fm-bootstrap.sh)) and prints one `AFK_READER:` line when away mode is active, delivery is paneless, this home's daemon is not confidently gone, the beacon is stale beyond the shared window, and records are genuinely waiting; a home whose daemon is confidently gone is left to the revive sweep instead, so the report never points a session at the wrong subsystem.
The daemon gate asks `fm_afk_daemon_owns_supervision`, so a liveness probe that cannot complete still counts as daemon-owned and the alarm still fires.
That is deliberate: `fm_pid_identity` returns non-zero whenever the process's `/proc` entry cannot be read or comes back truncated, and letting a transiently unreadable probe silence this detector would suppress the alarm in exactly the state it exists to catch.
...
So that the homes sharing the ledger also share one ceiling, set `FM_HEAVY_SLOTS_FILE` to the authoritative `config/heavy-run-slots` (the primary home's); when it is unset or unreadable the resolver falls back to this home's own `config/heavy-run-slots`, then to the highest ceiling recorded among live ledger entries, then to `1`.
Each run stamps its resolved ceiling into its own lease record, so a waiter whose env/config chain would otherwise fall back to bare `1` - the case of a heavy-run subprocess spawned with no config file and no pointer - adopts the real shared cap the other homes agreed on rather than capping concurrency at 1.
An explicit `FM_HEAVY_SLOTS` or a readable pointer/home config still wins; the ledger read only replaces the bare-`1` fallback, and a malformed recorded ceiling is ignored and falls to `1` loudly.
...
FM_STATUS_FILE_MAX_LINES=200   # max lines the live state/<id>.status file may reach via bin/fm-status-append.sh; after each append the older lines are archived to state/<id>.status.1 and the live file is trimmed (atomic rewrite) to its most-recent MAX lines, preserving the verb-keyed tail readers depend on; 0 disables the file cap (docs/scripts.md fm-status-append.sh)
